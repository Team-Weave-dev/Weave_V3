...
- usePlanLimits.ts 13~24 interface UsageLimits - Usage limits interface for tracking current usage and limits
- usePlanLimits.ts 26~106 export usePlanLimits - 요금제 제한 체크를 위한 React Hook Provides current plan, usage data, and functions to check if operations are allowed @returns { plan: PlanType, usage: UsageLimits, loading: boolean, canCreateProject: () => {allowed: boolean, message?: string}, canAddWidget: () => {allowed: boolean, message?: string}, refresh: () => void } @example ```tsx const { canCreateProject, refresh } = usePlanLimits(); const limitCheck = canCreateProject(); if (!limitCheck.allowed) { toast({ title: '프로젝트 생성 제한', description: limitCheck.message, variant: 'destructive' }); return; } ```
