    ...
    for (let row = 1; row <= maxRows; row++) {
      for (let col = 1; col <= columns - width + 1; col++) {
        ...
        for (const widget of widgets) {
          const wLeft = widget.position.gridColumnStart;
          const wRight = wLeft + (widget.size?.width || 2) - 1;
          const wTop = widget.position.gridRowStart;
          const wBottom = wTop + (widget.size?.height || 2) - 1;
          
          const testLeft = col;
          const testRight = col + width - 1;
          const testTop = row;
          const testBottom = row + height - 1;
          
          if (!(testRight < wLeft || testLeft > wRight || testBottom < wTop || testTop > wBottom)) {
            isEmpty = false;
            break;
          }
        }
